...
AAreaHazardExclusion::AAreaHazardExclusion()
{
	// デバッグ描画のためだけに Tick する（判定は Field からの ContainsPoint 呼び出し）
...
void AAreaHazardExclusion::OnConstruction( const FTransform& Transform )
{
...
bool AAreaHazardExclusion::ContainsPoint( const FVector& WorldPoint ) const
{
	return HazardAreaUtils::ContainsPoint( Shape, GetActorTransform(), BoxExtent, SphereRadius, WorldPoint );
}

void AAreaHazardExclusion::Tick( float DeltaTime )
{
