...
#if defined(_MSC_VER) && !defined(__clang__)
// MSVC's conformant preprocessor (/Zc:preprocessor) doesn't expand the
// __has_builtin(__make_integer_seq) token-paste trick from the block above,
// which leaves __has_builtin__make_integer_seq undefined here (MSVC C4668).
// __make_integer_seq has been available on MSVC since VS2015 Update 2, which
// is already the minimum supported by this header, so just assume it exists.
#define __BOOST_SML_HAS_MAKE_INTEGER_SEQ 1
#else
#define __BOOST_SML_HAS_MAKE_INTEGER_SEQ __has_builtin(__make_integer_seq)
#endif
#if __BOOST_SML_HAS_MAKE_INTEGER_SEQ
template <class T, T...>
...
template <class, class TDeps, class TEvent, class TState>
constexpr void log_state_snapshot(const aux::type_wrapper<no_policy> &, TDeps &, const TEvent &, const TState &) {}
template <class SM, class TLogger, class TDeps, class TEvent, class TState>
constexpr auto log_state_snapshot_impl(int, const aux::type_wrapper<TLogger> &, TDeps &deps, const TEvent &event, const TState &)
    -> decltype(static_cast<aux::pool_type<TLogger &> &>(deps).value.template log_state_snapshot<SM>(
                    event, get_state_name_t<TState>{}),
                void()) {
  return static_cast<aux::pool_type<TLogger &> &>(deps).value.template log_state_snapshot<SM>(event,
                                                                                               get_state_name_t<TState>{});
}
template <class SM, class TLogger, class TDeps, class TEvent, class TState>
constexpr void log_state_snapshot_impl(...) {}
template <class SM, class TLogger, class TDeps, class TEvent, class TState>
constexpr void log_state_snapshot(const aux::type_wrapper<TLogger> &wrapper, TDeps &deps, const TEvent &event,
                                  const TState &state) {
  (void)state;
  log_state_snapshot_impl<SM, TLogger, TDeps, TEvent, TState>(0, wrapper, deps, event, state);
}
...
struct sm_impl : aux::conditional_t<aux::should_not_subclass_statemachine_class<Tsm>::value, aux::none_type, typename Tsm::sm> {
  ...
  template <class T>
  struct is_sub_sm : aux::false_type {};
  template <class T>
  struct is_sub_sm<back::sm<T>> : aux::true_type {};
  template <class SM, class TEvent, class TDeps, class TSubs, class TState,
            __BOOST_SML_REQUIRES(!is_sub_sm<typename TState::type>::value)>
  constexpr void log_state_snapshot_recursive(const TEvent &event, TDeps &deps, const TSubs &, const TState &state) const {
    policies::log_state_snapshot<SM>(aux::type_wrapper<logger_t>{}, deps, event, state);
  }
  template <class SM, class TEvent, class TDeps, class TSubs, class TState,
            __BOOST_SML_REQUIRES(is_sub_sm<typename TState::type>::value)>
  constexpr void log_state_snapshot_recursive(const TEvent &event, TDeps &deps, const TSubs &subs, const TState &state) const {
    using sub_sm_impl_t = aux::apply_t<back::sm_impl, typename TState::type>;
    policies::log_state_snapshot<SM>(aux::type_wrapper<logger_t>{}, deps, event, state);
    back::sub_sm<sub_sm_impl_t>::cget(&subs).visit_current_states(
        [&](auto sub_state) {
          log_state_snapshot_recursive<typename sub_sm_impl_t::sm_t>(event, deps, subs, sub_state);
        },
        typename sub_sm_impl_t::states_t{}, aux::make_index_sequence<sub_sm_impl_t::regions>{});
  }
  template <class TEvent, class TDeps, class TSubs>
  constexpr void log_process_event_with_active_states(const TEvent &event, TDeps &deps, const TSubs &subs) const {
    policies::log_process_event<sm_t>(aux::type_wrapper<logger_t>{}, deps, event);
    visit_current_states(
        [&](auto state) {
          log_state_snapshot_recursive<sm_t>(event, deps, subs, state);
        },
        states_t{}, aux::make_index_sequence<regions>{});
  }
  ...
  constexpr bool process_internal_events(const TEvent &event, TDeps &deps, TSubs &subs) {
    log_process_event_with_active_states(event, deps, subs);
#if BOOST_SML_DISABLE_EXCEPTIONS
